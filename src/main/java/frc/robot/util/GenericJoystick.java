...
public class GenericJoystick {
    ...
    {
        directAxisMap = new HashMap<>() {{
            put(Axis.LX, 0);
            put(Axis.LY, 1);
            put(Axis.RX, 2);
            put(Axis.RY, 3);
        }};
        directButtonMap = new HashMap<>() {{
            put(Button.N, 4); // Y
            put(Button.E, 3); // B
            put(Button.S, 2); // A
            put(Button.W, 1); // X
    ...
    public JoystickButton getButton(Button button) {
        return switch (base) {
            case DIRECT -> new JoystickButton(direct, directButtonMap.get(button));
